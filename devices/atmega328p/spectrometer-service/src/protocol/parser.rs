...
static SERIES_REGEX: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^SERIES(\d)\s*=\s*(?:\[([^\]]+)\]|([0-9][0-9 ]*))").unwrap());
...
mod tests {
    ...
    #[test]
    fn test_parse_series_bare_format() {
        // Raw putty.log format without brackets
        let result = parse_line("SERIES1 = 16777215 16777215 16777215");
        assert_eq!(
            result,
            ParsedLine::Series {
                number: 1,
                values: vec![16777215, 16777215, 16777215]
            }
        );

        let result = parse_line("SERIES2 = 0 213 7");
        assert_eq!(
            result,
            ParsedLine::Series {
                number: 2,
        ...
        let result = parse_line("SERIES3 = 13109129 13080972 13105007");
        assert_eq!(
            result,
            ParsedLine::Series {
                number: 3,
                values: vec![13109129, 13080972, 13105007]
            }
        );
    }

    #[test]
    fn test_parse_ok_gain() {
        assert_eq!(parse_line("OK GAIN=1"), ParsedLine::GainSet(1));
        assert_eq!(parse_line("OK GAIN=128"), ParsedLine::GainSet(128));
    }
